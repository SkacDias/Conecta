import { emailsData } from '@/data/emailsData';
	...
	function componente({maxVisible} : EmailTruncationProps) {
		interface RowData {
			id: number;
		...
		const maxEmailsVisiveis = 5;
		const EmailsInvisiveis = emailsData.length-maxEmailsVisiveis;
	}

	const EmailTable: React.FC<TableProps>  = () => {
		return (
				...
				{emailsData.map((email, index) => (
				  <tr key={index}>
					...
					<td className='border p-2 w-1/5 overflow-hidden '>
					  <div className='w-10'>{{truncarEmails({email.recipients, maxVisible: maxEmailsVisiveis })}}</div>
					  <div className='tooltip'>EmailsInvisiveis</div>
					</td>
